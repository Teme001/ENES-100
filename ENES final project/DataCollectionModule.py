...
myDirectory = os.path.join(os.getcwd(), 'Collected Data')


# CREATE A NEW FOLDER BASED ON THE PREVIOUS FOLDER COUNT
while os.path.exists(os.path.join(myDirectory,f'IMG{str(FolderCounter)}')):
        FolderCounter += 1
newPath = myDirectory +"/IMG"+str(FolderCounter)
os.makedirs(newPath)
...
def saveLog():
    ...
    df.to_csv(os.path.join(myDirectory,f'log_{str(FolderCounter)}.csv'), index=False, header=False)
    print('Log Saved')
